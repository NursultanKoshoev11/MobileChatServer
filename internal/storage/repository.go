...
func (r *Repository) ListGroupMembers(ctx context.Context, groupID, userID string) ([]domain.GroupMember, error) {
	isMember, err := r.IsGroupMember(ctx, groupID, userID)
	if err != nil {
		return nil, err
	}
	...
	query := `
		...
		WHERE gm.group_id = $1
		ORDER BY CASE gm.role WHEN 'owner' THEN 1 WHEN 'admin' THEN 2 ELSE 3 END, lower(u.display_name)`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&member.UserID, &member.DisplayName, &member.Phone, &member.Role); err != nil {
...
func (r *Repository) UpdateGroupMemberRole(ctx context.Context, groupID, actorID, targetUserID string, role domain.GroupRole) (domain.GroupMember, error) {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return domain.GroupMember{}, fmt.Errorf("begin update group member role: %w", err)
	}
	defer tx.Rollback(ctx)
	...
	if err := tx.QueryRow(ctx, `SELECT role FROM group_members WHERE group_id=$1 AND user_id=$2 FOR UPDATE`, groupID, targetUserID).Scan(&currentRole); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return domain.GroupMember{}, ErrNotFound
		}
		return domain.GroupMember{}, fmt.Errorf("load target group role: %w", err)
	}
	if currentRole == domain.RoleOwner {
		return domain.GroupMember{}, ErrForbidden
	}

	if _, err := tx.Exec(ctx, `UPDATE group_members SET role=$3 WHERE group_id=$1 AND user_id=$2`, groupID, targetUserID, role); err != nil {
		return domain.GroupMember{}, fmt.Errorf("update group member role: %w", err)
	}
	...
	if err := tx.QueryRow(ctx, query, groupID, targetUserID).Scan(&member.UserID, &member.DisplayName, &member.Phone, &member.Role); err != nil {
		return domain.GroupMember{}, fmt.Errorf("load updated group member: %w", err)
	}
	if err := tx.Commit(ctx); err != nil {
		return domain.GroupMember{}, fmt.Errorf("commit update group member role: %w", err)
	}
	return member, nil
}
